...
def create_shelf():
    shelf = ub.Shelf()
    if request.args.get('is_public') == '1':
        shelf.is_public = 1
...
@shelf.route("/shelf/api/info/<int:shelf_id>")
@user_login_required
def api_shelf_info(shelf_id):
    shelf = ub.session.query(ub.Shelf).filter(ub.Shelf.id == shelf_id).first()
    if shelf and check_shelf_view_permissions(shelf):
        return jsonify({"name": shelf.name, "is_public": shelf.is_public})
    return jsonify({"error": "Not found"}), 404


@shelf.route("/shelf/create_smart", methods=["POST"])
@user_login_required
def create_smart():
    ...
    # --- NEW: BAKE ADVANCED SEARCH CACHE INTO URL ---
    if "/advsearch" in search_url:
        from flask import session as flask_session
        from urllib.parse import quote
        current_query = flask_session.get('query', '{}')
        base_url = search_url.split("?")[0]
        search_url = f"{base_url}?query={quote(current_query)}"
    # ------------------------------------------------
    ...
    if check_shelf_is_unique(name, is_public):
        shelf = ub.Shelf(name=name, is_public=is_public, user_id=int(current_user.id))
        shelf.search_args = search_url
        try:
            ub.session.add(shelf)
            ub.session.commit()
            flash(_("Smart Shelf '%(title)s' created", title=name), category="success")
            separator = "&" if "?" in search_url else "?"
            return redirect(f"{search_url}{separator}smart_shelf={shelf.id}")
...
@shelf.route("/shelf/edit_smart/<int:shelf_id>", methods=["POST"])
@user_login_required
def edit_smart(shelf_id):
    shelf = ub.session.query(ub.Shelf).filter(ub.Shelf.id == shelf_id).first()
    if not check_shelf_edit_permissions(shelf):
        flash(_("Sorry you are not allowed to edit this shelf"), category="error")
        return redirect(url_for('web.index'))
    ...
    if name and check_shelf_is_unique(name, is_public, shelf_id):
        shelf.name = name
        shelf.is_public = is_public
        try:
            ub.session.commit()
            flash(_("Smart Shelf updated successfully"), category="success")
        except Exception as ex:
            ub.session.rollback()
            log.error_or_exception(ex)
            flash(_("There was an error"), category="error")

    search_url = shelf.search_args
    if search_url:
        separator = "&" if "?" in search_url else "?"
        return redirect(f"{search_url}{separator}smart_shelf={shelf.id}")
    return redirect(url_for('web.index'))
...
def render_show_shelf(shelf_type, shelf_id, page_no, sort_param):
    ...
    if shelf and check_shelf_view_permissions(shelf):
        ...
        # --- NEW: Per-Shelf Stacking ---
        stack_pref = current_user.get_view_property("shelf_stack", str(shelf_id))
        stack = (stack_pref is None or stack_pref == 1)
        
        # --- FIX: DISABLE STACKING FOR THE WISHLIST ---
        ...
        if shelf_type == 1:
            ...
            dynamic_sort = []
            if sort_param == 'pubnew': dynamic_sort = [db.Books.pubdate.desc()]
            elif sort_param == 'pubold': dynamic_sort = [db.Books.pubdate.asc()]
            elif sort_param == 'abc': dynamic_sort = [db.Books.sort.asc()]
            elif sort_param == 'zyx': dynamic_sort = [db.Books.sort.desc()]
            elif sort_param == 'new': dynamic_sort = [db.Books.timestamp.desc()]
            elif sort_param == 'old': dynamic_sort = [db.Books.timestamp.asc()]
            elif sort_param == 'authaz': dynamic_sort = [db.Books.author_sort.asc()]
            elif sort_param == 'authza': dynamic_sort = [db.Books.author_sort.desc()]
            else: 
                dynamic_sort = [ub.BookShelf.order.asc()]
        ...
        if stack or series_first:
            query = query.outerjoin(db.books_series_link, db.books_series_link.c.book == db.Books.id).outerjoin(db.Series)
        
        query = query.filter(ub.BookShelf.shelf == shelf_id)
        query = query.filter(calibre_db.common_filters(True))

        if stack:
            # --- FIX: FORCE LOWEST SERIES INDEX COVER ---
            query = query.add_columns(func.min(db.Books.series_index))
            query = query.group_by(func.coalesce(db.Series.id, db.Books.id * -1))

        if series_first:
            sort_order = [db.Series.name == None, db.Series.name, db.Books.series_index] + dynamic_sort
        else:
            if stack:
                sort_order = dynamic_sort + [db.Series.name, db.Books.series_index]
            else:
                sort_order = dynamic_sort

        query = query.order_by(*sort_order)
        off = int(int(pagesize) * (page_no - 1))
        
        if pagesize == sys.maxsize:
            pagination = Pagination(page_no, pagesize, query.count())
            result = query.all()
        else:
            pagination = Pagination(page_no, pagesize, query.count())
            result = query.offset(off).limit(pagesize).all()
            
        result = calibre_db.order_authors(result, True, True)
        ...
        if shelf.is_public:
             display_title = _("Public Shelf: '%(name)s'", name=shelf.name)
        else:
             display_title = _("Private Shelf: '%(name)s'", name=shelf.name)

       # --- NEW: FILTER DUPLICATES IN PYTHON AND EXTRACT GLOBAL COUNTS ---
        series_counts = {}
        if stack:
            stacked_result = []
            seen_series = set()
            for row in result:
                # Safely extract the actual Book object from the SQLAlchemy Row tuple
                actual_book = row.Books if hasattr(row, 'Books') else (row[0] if isinstance(row, tuple) else row)
                
                if actual_book.series:
                    s_name = actual_book.series[0].name
                    if s_name not in seen_series:
                        seen_series.add(s_name)
                        stacked_result.append(row)
                        
                        # --- FIX: GLOBAL SERIES COUNT ---
                        # Instead of counting books on the shelf, run a quick global count
                        # identical to what the Series destination page uses.
                        count = calibre_db.session.query(db.Books)\
                            .join(db.books_series_link, db.Books.id == db.books_series_link.c.book)\
                            .filter(db.books_series_link.c.series == actual_book.series[0].id)\
                            .filter(calibre_db.common_filters(True))\
                            .count()
                            
                        series_counts[actual_book.series[0].id] = count
                else:
                    stacked_result.append(row)
...
@shelf.route("/shelf/toggle_stack/<int:shelf_id>", methods=["POST"])
@user_login_required
def toggle_stack(shelf_id):
    shelf_str = str(shelf_id)
    # Get the current setting (defaults to 1 if not set)
    current = current_user.get_view_property("shelf_stack", shelf_str)
    new_val = 0 if (current == 1 or current is None) else 1
    
    # Save the new preference to the user's database row
    current_user.set_view_property("shelf_stack", shelf_str, new_val)
