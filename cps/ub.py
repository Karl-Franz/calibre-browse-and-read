...
class Annotation(Base):
    ...
    id = Column(Integer, primary_key=True)
    book_id = Column(Integer)  # Links to the Calibre book ID
    user_id = Column(Integer, ForeignKey('user.id'))
    format = Column(String(20))  # e.g., 'EPUB' or 'PDF'
    cfi = Column(String(255))  # The exact EpubCFI range of the highlight
    highlight_text = Column(String)  # The actual text that was highlighted
    note = Column(String)  # For custom bookmark names or user notes
    color = Column(String(20), default='yellow')  # Highlight color (yellow, green, pink, etc.)
    timestamp = Column(DateTime)

    user = relationship('User', backref='annotations')

    def __init__(self, book_id, user_id, format, cfi, highlight_text, note="", color="yellow"):
