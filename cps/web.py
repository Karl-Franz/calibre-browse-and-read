...
# --- CUSTOM USER RATINGS ROUTE ---
@web.route("/ajax/set_user_rating/<int:book_id>", methods=["POST"])
@user_login_required
def set_user_rating(book_id):
    new_rating = request.form.get('rating', type=int)
    
    if new_rating is None:
        return "Missing rating", 400

    existing_rating = ub.session.query(ub.UserRating).filter(
        ub.UserRating.user_id == current_user.id,
        ub.UserRating.book_id == book_id
    ).first()

    if existing_rating:
        existing_rating.rating = new_rating
    else:
        new_entry = ub.UserRating(user_id=current_user.id, book_id=book_id, rating=new_rating)
        ub.session.add(new_entry)
        
    ub.session.commit()
    return "Success", 200
# ---------------------------------

@web.route("/ajax/save_progress/<int:book_id>", methods=["POST"])
@user_login_required
def save_progress(book_id):
    location_val = request.form.get("location", "")
    progress_pct = request.form.get("progress", 0.0)
    settings_val = request.form.get("settings", "")
    book_format = request.form.get("format", "EPUB").upper()

    if location_val:
        state = ub.session.query(ub.KoboReadingState).filter(
            ub.KoboReadingState.user_id == current_user.id,
            ub.KoboReadingState.book_id == book_id
        ).first()

        if not state:
            state = ub.KoboReadingState(user_id=current_user.id, book_id=book_id)
            ub.session.add(state)
            ub.session.commit()

        bookmark = ub.session.query(ub.KoboBookmark).filter(
            ub.KoboBookmark.kobo_reading_state_id == state.id
        ).first()

        if not bookmark:
            bookmark = ub.KoboBookmark(
                kobo_reading_state_id=state.id,
                location_type="epubcfi" if "epubcfi" in location_val else "page"
            )
            ub.session.add(bookmark)

        bookmark.location_value = location_val
        try:
            bookmark.progress_percent = float(progress_pct)
        except ValueError:
            bookmark.progress_percent = 0.0
            
        state.current_bookmark_id = bookmark.id

    try:
        settings_dict = json.loads(settings_val) if settings_val else {}
    except:
        settings_dict = {}
        
    settings_dict['last_opened'] = int(time.time())
    settings_val = json.dumps(settings_dict)

    settings_bkmk = ub.session.query(ub.Bookmark).filter(
        ub.Bookmark.user_id == current_user.id,
        ub.Bookmark.book_id == book_id,
        ub.Bookmark.format == book_format
    ).first()

    if not settings_bkmk:
        settings_bkmk = ub.Bookmark(
            user_id=current_user.id,
            book_id=book_id,
            format=book_format,
            bookmark_key=settings_val
        )
        ub.session.add(settings_bkmk)
    else:
        settings_bkmk.bookmark_key = settings_val

    ub.session.commit()
    return "OK", 200

@web.route("/ajax/remove_currently_reading/<int:book_id>", methods=["POST"])
@user_login_required
def remove_currently_reading(book_id):
    state = ub.session.query(ub.KoboReadingState).filter(
        ub.KoboReadingState.user_id == current_user.id,
        ub.KoboReadingState.book_id == book_id
    ).first()
    ...
    if state:
        ub.session.query(ub.KoboBookmark).filter(ub.KoboBookmark.kobo_reading_state_id == state.id).delete()
        ub.session.delete(state)
        ub.session.commit()
        
    return "OK", 200

@web.route("/ajax/save_annotation/<int:book_id>", methods=["POST"])
@user_login_required
def save_annotation(book_id):
    format = request.form.get("format", "EPUB").upper()
    cfi = request.form.get("cfi")
    highlight_text = request.form.get("text", "")
    note = request.form.get("note", "")
    color = request.form.get("color", "yellow")

    if not cfi:
        return "Missing location data", 400

    annotation = ub.session.query(ub.Annotation).filter(
        ub.Annotation.user_id == current_user.id,
        ub.Annotation.book_id == book_id,
        ub.Annotation.format == format,
        ub.Annotation.cfi == cfi
    ).first()
    ...
    if not annotation:
        ...
        annotation.highlight_text = highlight_text 

    ub.session.commit()
    return "OK", 200


@web.route("/ajax/get_annotations/<int:book_id>/<format>", methods=["GET"])
@user_login_required
def get_annotations(book_id, format):
    annotations = ub.session.query(ub.Annotation).filter(
        ub.Annotation.user_id == current_user.id,
        ub.Annotation.book_id == book_id,
        ub.Annotation.format == format.upper()
    ).all()
...
@web.route("/ajax/delete_annotation/<int:annotation_id>", methods=["POST"])
@user_login_required
def delete_annotation(annotation_id):
    annotation = ub.session.query(ub.Annotation).filter(
        ub.Annotation.id == annotation_id,
        ub.Annotation.user_id == current_user.id
    ).first()
    
    if annotation:
        ub.session.delete(annotation)
        ub.session.commit()
        return "OK", 200
...
def generate_char_list(entries): 
    char_list = list()
...
def get_sort_function(sort_param, data):
    ...
    if sort_param == 'series_order':
        order = [db.Series.name == None, db.Series.name, db.Books.series_index]
...
def render_currently_reading_books(page, order):
    currently_reading_books = []
    if current_user.is_authenticated:
        try:
            states = ub.session.query(ub.KoboReadingState)\
                .filter(ub.KoboReadingState.user_id == current_user.id).all()
            
            if states:
                book_ids = [state.book_id for state in states]
                books_query = calibre_db.session.query(db.Books).filter(db.Books.id.in_(book_ids)).all()
                book_map = {b.id: b for b in books_query}
                
                bookmarks = ub.session.query(ub.Bookmark).filter(ub.Bookmark.user_id == current_user.id).all()
                
                last_opened_map = {}
                for b in bookmarks:
                    try:
                        b_dict = json.loads(b.bookmark_key)
                        t = b_dict.get('last_opened', 0)
                        if t > last_opened_map.get(b.book_id, 0):
                            last_opened_map[b.book_id] = t
                    except:
                        pass
                
                # --- NEW: Fetch True Read Statuses for the Dedicated Grid ---
                read_states = ub.session.query(ub.ReadBook).filter(
                    ub.ReadBook.user_id == current_user.id,
                    ub.ReadBook.book_id.in_(book_ids)
                ).all()
                read_map = {rb.book_id: (rb.read_status == ub.ReadBook.STATUS_FINISHED) for rb in read_states}
                
                for state in states:
                    book = book_map.get(state.book_id)
                    if book:
                        prog = 0.0
                        if getattr(state, 'current_bookmark', None):
                            prog = state.current_bookmark.progress_percent or 0.0
                            
                        book.reading_progress_pct = int(prog * 100) if prog <= 1.0 else int(prog)
                        book.reading_progress_pct = max(0, min(100, book.reading_progress_pct))
                        
                        book.last_opened_ts = last_opened_map.get(book.id, state.id)
                        book.read_status = read_map.get(book.id, False) # Assign read status
                        currently_reading_books.append(book)
                        
                currently_reading_books.sort(key=lambda x: getattr(x, 'last_opened_ts', 0), reverse=True)
        except Exception as e:
            log.error(f"Error fetching currently reading books: {e}")

    pagesize = config.config_books_per_page
    off = int(int(pagesize) * (page - 1))
    total = len(currently_reading_books)
    pagination = Pagination(page, pagesize, total)
    entries = currently_reading_books[off:off+pagesize]
    
    # Safe mapping for index.html compatibility
    safe_entries = [(book, False, getattr(book, 'read_status', False)) for book in entries]

    return render_title_template('index.html', random=false(), entries=safe_entries, pagination=pagination,
                                 title=_("Currently Reading"), page="currently_reading", order=order[1])
...
def render_books_list(data, sort_param, book_id, page):
    ...
    if data == "rated":
    ...
    elif data == "currently_reading":
        return render_currently_reading_books(page, order)
    elif data == "discover":
        ...
        return render_boxsets_books(page, order)
    elif data == "search":
        term = request.args.get('query', None)
        offset = int(int(config.config_books_per_page) * (page - 1))
        return render_search_results(term, offset, order, config.config_books_per_page)
    elif data == "advsearch":
        # --- NEW: ADVANCED SEARCH SMART SHELF FIX ---
        # Check if the URL explicitly provides the JSON query (from a Smart Shelf link)
        url_query = request.args.get('query')
        if url_query and url_query.strip() and url_query != "{}":
            # Rehydrate the session so pagination and future clicks remember the query!
            flask_session['query'] = url_query
            term = json.loads(url_query)
        else:
        ...
        currently_reading_books = []
        if website == "newest" and current_user.is_authenticated:
            try:
                states = ub.session.query(ub.KoboReadingState)\
                    .filter(ub.KoboReadingState.user_id == current_user.id).all()
                ...
                if states:
                    ...
                    # --- NEW: Fetch True Read Statuses for Side-Scroller ---
                    read_states = ub.session.query(ub.ReadBook).filter(
                        ub.ReadBook.user_id == current_user.id,
                        ub.ReadBook.book_id.in_(book_ids)
                    ).all()
                    ...
                    currently_reading_books.sort(key=lambda x: getattr(x, 'last_opened_ts', 0), reverse=True)
                    currently_reading_books = currently_reading_books[:100] 
            except Exception as e:
                log.error(f"Error fetching currently reading books: {e}")
        ...
        if stack:
            ...
            query = query.add_columns(func.min(db.Books.series_index))
            
            entries = query.offset(off).limit(pagesize).all()
            entries = calibre_db.order_authors(entries, True, True)
            
            if current_user.show_detail_random() and website != "discover":
...
def render_rated_books(page, book_id, order):
    if current_user.check_visibility(constants.SIDEBAR_BEST_RATED):
        ...
        if stack:
            ...
            query = query.group_by(func.coalesce(db.Series.id, db.Books.id * -1))
            query = query.order_by(*sort_order)
...
def render_boxsets_books(page, order):
    entries, random, pagination = calibre_db.fill_indexpage(page, 0,
                                                            db.Books,
                                                            db.Books.tags.any(db.Tags.name.ilike("Box Set")),
                                                            order[0],
                                                            True, config.config_read_column,
                                                            db.books_series_link,
                                                            db.Books.id == db.books_series_link.c.book,
    ...
    return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
                                 title=_("Box Sets"), page="boxsets", order=order[1])
...
def render_author_books(page, author_id, order):
    ...
    if series_first:
        ...
        sort_order = [order[0][0], db.Series.name, db.Books.series_index]

    if stack:
        query = calibre_db.generate_linked_query(config.config_read_column, db.Books)
        query = query.outerjoin(db.books_series_link, db.books_series_link.c.book == db.Books.id).outerjoin(db.Series)
        query = query.filter(db.Books.authors.any(db.Authors.id == author_id))
        query = query.filter(calibre_db.common_filters(True))
...
def render_category_books(page, book_id, order):
    ...
    if book_id == '-1':
        ...
        if stack:
            query = calibre_db.generate_linked_query(config.config_read_column, db.Books)
            query = query.outerjoin(db.books_series_link, db.books_series_link.c.book == db.Books.id).outerjoin(db.Series)
            query = query.outerjoin(db.books_tags_link, db.books_tags_link.c.book == db.Books.id).outerjoin(db.Tags)
            query = query.filter(db.Tags.name == None)
            query = query.filter(calibre_db.common_filters(True))
        ...
        tag_entry = calibre_db.session.query(db.Tags).filter(db.Tags.id == book_id).first()
        if tag_entry:
            tagsname = tag_entry.name
            cat_filter = db.Books.tags.any(
                or_(
                    db.Tags.id == book_id,
                    db.Tags.name.like(tag_entry.name + ".%")
                )
            )
...
def ratings_list():
    if current_user.check_visibility(constants.SIDEBAR_RATING):
        ...
        if not current_user.is_anonymous:
            u_order = ub.UserRating.rating.desc() if order_no == 0 else ub.UserRating.rating.asc()
            raw_user_ratings = ub.session.query(
                ub.UserRating.rating, 
                func.count(ub.UserRating.book_id).label('count')
            ).filter(
                ub.UserRating.user_id == current_user.id,
                ub.UserRating.rating > 0
            ).group_by(ub.UserRating.rating).order_by(u_order).all()

            for r in raw_user_ratings:
                # We hijack the standard Category class to format personal ratings for the HTML
                fake_cat = db.Category(f"{int(r.rating/2)}", str(r.rating), r.rating)
                fake_cat.rating = r.rating
                user_entries.append([fake_cat, r.count])

        return render_title_template('list.html', entries=entries, user_entries=user_entries, folder='web.books_list', charlist=list(),
                                     title=_("Ratings list"), page="ratingslist", data="ratings", order=order_no)
    else:
        abort(404)

# --- 3. NEW ROUTE: GRID VIEW FOR PERSONAL RATINGS ---
@web.route('/user_ratings/<sort_param>/<int:book_id>', defaults={'page': 1})
@web.route('/user_ratings/<sort_param>/<int:book_id>/<int:page>')
@login_required_if_no_ano
def user_ratings_books(sort_param, book_id, page):
    order = get_sort_function(sort_param, "user_ratings")
    
    # Get all books the user gave this specific rating to
    user_ratings = ub.session.query(ub.UserRating.book_id).filter(
        ub.UserRating.user_id == current_user.id,
        ub.UserRating.rating == book_id
    ).all()
    ...
    title = _("My Rating: %(rating)s stars", rating=int(book_id / 2))
    return render_title_template('index.html', random=random, pagination=pagination, entries=entries, id=book_id,
                                 title=title, page="user_ratings", order=order[1])
...
def read_book(book_id, book_format):
    ...
    if current_user.is_authenticated:
        ...
        if state and state.current_bookmark:
            saved_location = state.current_bookmark.location_value

    if book_format.lower() == "epub" or book_format.lower() == "kepub":
        log.debug("Start [k]epub reader for %d", book_id)
        return render_title_template('read.html', bookid=book_id, title=book.title, bookmark=bookmark,
                                     book_format=book_format, saved_location=saved_location)
    elif book_format.lower() == "pdf":
        log.debug("Start pdf reader for %d", book_id)
        return render_title_template('readpdf.html', pdffile=book_id, title=book.title, saved_location=saved_location)
...
def show_book(book_id):
    ...
    if entries:
        ...
        # Look up the current user's rating for this specific book
        user_rating_val = 0
        if current_user.is_authenticated:
            u_rating = ub.session.query(ub.UserRating).filter(
                ub.UserRating.user_id == current_user.id,
                ub.UserRating.book_id == book_id
            ).first()
            if u_rating:
                user_rating_val = u_rating.rating

        html_content = render_title_template('detail.html',
                                     entry=entry,
                                     cc=cc,
...
def inject_global_template_vars():
    ...
    if current_user.is_authenticated:
        # 1. Wishlist Lookup
        try:
            wishlist_shelf = ub.session.query(ub.Shelf).filter(
                ub.Shelf.user_id == int(current_user.id),
                ub.Shelf.name.ilike('%wishlist%')
            ).first()
            if wishlist_shelf:
                links = ub.session.query(ub.BookShelf).filter(ub.BookShelf.shelf == wishlist_shelf.id).all()
                wishlist_ids = [int(link.book_id) for link in links]
        except Exception as e:
            error_msg = "Wishlist DB Error: " + str(e)
            pass
            
        # 2. Progress Lookup
        try:
            states = ub.session.query(ub.KoboReadingState).filter(ub.KoboReadingState.user_id == int(current_user.id)).all()
            for state in states:
                prog = 0.0
                if getattr(state, 'current_bookmark', None):
                    prog = state.current_bookmark.progress_percent or 0.0
                
                pct = int(prog * 100) if prog <= 1.0 else int(prog)
                progress_dict[int(state.book_id)] = max(0, min(100, pct))
        except Exception as e:
            error_msg = "Progress DB Error: " + str(e)
            pass

        # 3. User Ratings Lookup
        try:
            ratings = ub.session.query(ub.UserRating).filter(ub.UserRating.user_id == int(current_user.id)).all()
            for r in ratings:
                if r.rating > 0:
                    user_ratings_dict[int(r.book_id)] = r.rating
        except Exception as e:
            error_msg = "Ratings DB Error: " + str(e)
            pass
            
    return dict(
        global_wishlist_ids=wishlist_ids, 
        global_reading_progress=progress_dict,
        global_user_ratings=user_ratings_dict,
        global_debug_error=error_msg
    )