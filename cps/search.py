...
def adv_search_tag(q, include_tag_inputs, exclude_tag_inputs, tag_match_mode='all'):
    if include_tag_inputs:
        if tag_match_mode == 'any':
            # OR logic: Book must have ANY of the selected tags
            q = q.filter(db.Books.tags.any(db.Tags.id.in_(include_tag_inputs)))
        else:
            # AND logic: Book must have ALL selected tags
...
def extend_search_term(searchterm,
    ...
    searchterm = joiner.join(filter(None, searchterm))
...
def render_adv_search_results(term, offset=None, order=None, limit=None):
    ...
    if any(tags.values()) or author_name or book_title or publisher or pub_start or pub_end or rating_low \
       or rating_high or description or cc_present or read_status != "Any":
        match_mode = term.get('tag_match_mode', 'all')
        ...
        # Extract the toggle value (defaults to 'all' if missing)
        tag_match_mode = term.get('tag_match_mode', 'all')
        q = adv_search_tag(q, tags['include_tag'], tags['exclude_tag'], tag_match_mode)
